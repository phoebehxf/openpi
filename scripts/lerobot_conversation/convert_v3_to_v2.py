...
def convert_data(
    ...
    for (chunk_idx, file_idx), records in tqdm.tqdm(grouped.items(), desc="convert data files"):
        ...
        for record in records:
            ...
            # Drop stale v3 Hugging Face schema metadata before writing the v2 parquet.
            # The older `datasets` version used by OpenPI cannot parse the v3 feature
            # descriptors (for example `_type: "List"`), and it should infer schema
            # from the Arrow table instead.
            episode_table = episode_table.replace_schema_metadata(None)
...
def _extract_video_segment(
    ...
    # Re-encode each segment instead of stream-copying it. Stream copy aligns cuts
    # to keyframes, which can shift the first decodable frame away from t=0 for the
    # per-episode clip and causes LeRobot's strict timestamp checks to fail.
    cmd = [
        "ffmpeg",
        "-hide_banner",
        "-loglevel",
